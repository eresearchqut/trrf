...
class AuthFailedBackendTest(TestCase):
    ...
    def test_authenticate_request_logs_username(self):
        ...
        self.assertTrue(
            is_recent(log.timestamp), "Should have logged it recently"
        )
...
class AuthFailedBackendEndToEndTest(TestCase):
    ...
    def login(self, **headers):
        all_headers = (
            ...
            if not headers
            else chain_maps(headers, self.DEFAULT_HEADERS)
        )
    ...
    def test_long_user_agent_is_truncated(self):
        user_agent_field_length = m.FailedLoginLog._meta.get_field(
            "user_agent"
        ).max_length
        long_user_agent = (
            "x" * user_agent_field_length
    ...
    def test_authenticate_request_logs_proxies(self):
        ...
        self.assertEqual(log.ip_address, "192.168.1.2")
        self.assertEqual(
            log.forwarded_by, "192.168.1.100,20.20.20.20,10.10.10.10"
        )
