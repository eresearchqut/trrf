...
class Log(models.Model):
    ...
    ip_address = models.CharField(
        max_length=40, null=True, blank=True, verbose_name="IP"
    )
...
class UserDeactivation(models.Model):
    ...
    reason = models.CharField(
        max_length=2, blank=True, null=True, choices=DEACTIVATION_REASON_CHOICES
    )
    timestamp = models.DateTimeField(auto_now_add=True)
...
class LoginLogger(object):
    ...
    def extract_log_info(self, username, request):
        ...
        if user_agent is not None and len(user_agent) > USER_AGENT_MAX_LENGTH:
            logger.warning(
                'Truncating User Agent to fit into %d. Original was: "%s"',
...
password_expired_callback = save_login_deactivation(
    UserDeactivation.PASSWORD_EXPIRED
)
account_expired_callback = save_login_deactivation(
    UserDeactivation.ACCOUNT_EXPIRED
)
login_failure_limit_reached_callback = save_login_deactivation(
