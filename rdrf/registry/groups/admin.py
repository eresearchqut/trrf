...
class CustomUserAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        ('Permissions', {
            'fields': ('is_active', 'require_2_fact_auth', 'prevent_self_unlock',
                       'is_staff', 'groups', 'registry', 'working_groups')
        })
    )
    ...
    def superuser_fieldsets(self, user):
        def get_permision_fields(user):
            ...
            show_ethically_cleared = any(r.has_feature(RegistryFeatures.CLINICIAN_ETHICAL_CLEARANCE)
                                         for r in user.registry.all())
