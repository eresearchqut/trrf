...
class CustomUserAdmin(UserAdmin):
    ...
    fieldsets = (
        ...
        ('Permissions', {
         'fields': ('is_active', 'require_2_fact_auth', 'force_password_reset', 'prevent_self_unlock', 'is_staff', 'groups', 'registry', 'working_groups')}))

    # only superusers are allowed to make users superuser
    def superuser_fieldsets(self, user):
        def get_permision_fields(user):
            base_fields = ['is_active', 'require_2_fact_auth', 'force_password_reset', 'prevent_self_unlock', 'is_staff', 'is_superuser',
