...
def load_report_configuration():
    ...
    get_report_config_func = getattr(
        report_config_module, settings.REPORT_CONFIG_METHOD_GET
    )
    ...
    def resolve_field_value(result, field_pointers):
        if result and field_pointers:
            ...
            result = resolve_field_value(
                result.get(next_pointer), field_pointers
            )

        return result

    graph_field_pointers = get_flattened_json_path(graphql_query_field).split(
        "_"
    )
