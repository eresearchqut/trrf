...
class ClinicalDataGeneratorTestCase(TestCase):
    ...
    def setUpTestData(cls):
        ...
        registry_definition = [
                ...
                "sections": [
                    {
                        ...
                        "cdes": [
                            ...
                            {
                                "code": "fatigue",
                                "abbreviated_name": "Fatigue",
                                "name": "Fatigue",
                            },
                            {
                                "code": "pain",
                                "abbreviated_name": "Pain",
                                "name": "Pain",
                            },
                        ],
                    },
                    {
                        "code": "otherSymptoms",
                        "abbreviated_name": "Other Symptoms",
                        "display_name": "Other Symptoms",
                        "cdes": [
                            {
                                "code": "energy",
                                "abbreviated_name": "Energy",
                                "name": "Energy",
                            }
                        ],
                    },
                ...
                "sections": [
                    {
                        ...
                        "cdes": [
                            ...
                            {
                                "code": "difficulty",
                                "abbreviated_name": "Difficulty",
                                "name": "Any difficulties",
                            },
                        ],
                    }
                ],
            ...
            {
                "name": "infancy",
                "abbreviated_name": "Infancy",
                "sections": [
                    {
                        "code": "growth",
                        "abbreviated_name": "Growth",
                        "display_name": "Growth and Development",
                        "cdes": [
                            {
                                "code": "weight6mo",
                                "abbreviated_name": "Weight at 6mo",
                                "name": "Weight at 6 months old",
                            }
                        ],
                    }
                ],
        ...
        for form in registry_definition:
            for section in form["sections"]:
                for cde in section["cdes"]:
                    CommonDataElement.objects.create(
                        code=cde["code"],
                        abbreviated_name=cde["abbreviated_name"],
                        name=cde["name"],
                        allow_multiple=cde.get("allow_multiple", False),
                    )
            ...
            RegistryForm.objects.create(
                ...
                abbreviated_name=form["abbreviated_name"],
                sections=",".join(
                    [section["code"] for section in form["sections"]]
                ),
            )

        # Context Form Groups
        cls.cfg_clinical_visit = ContextFormGroup.objects.create(
            code="clinicalVisit",
            registry=cls.registry,
            name="Clinical Visit",
            context_type="F",
        ...
        cls.cfg_clinical_visit.items.create(
            registry_form=RegistryForm.objects.get(name="myHealth")
        )
        cls.cfg_clinical_visit.items.create(
            registry_form=RegistryForm.objects.get(name="myBreathing")
        )
        cls.cfg_clinical_visit.items.create(
            registry_form=RegistryForm.objects.get(name="myAppointments")
        )
        cls.cfg_clinical_visit.items.create(
            registry_form=RegistryForm.objects.get(name="infancy")
        )

        cls.cfg_symptoms = ContextFormGroup.objects.create(
            code="symptoms",
            registry=cls.registry,
            name="Symptoms",
            context_type="M",
        ...
        cls.cfg_symptoms.items.create(
            registry_form=RegistryForm.objects.get(name="recentSymptoms")
        )

        cls.cfg_sleep = ContextFormGroup.objects.create(
            code="sleepTracking",
            registry=cls.registry,
            name="Sleep Tracking",
            context_type="F",
        ...
        cls.cfg_sleep.items.create(
            registry_form=RegistryForm.objects.get(name="sleep")
        )

        cls.cfg_tracking = ContextFormGroup.objects.create(
            code="ongoing",
            registry=cls.registry,
            name="Clinical Visit",
            context_type="M",
            abbreviated_name="initial",
            sort_order=4,
        )
        cls.cfg_tracking.items.create(
            registry_form=RegistryForm.objects.get(name="myHealth")
        )
        ...
        p1_ctx1 = RDRFContext.objects.create(
            context_form_group=cls.cfg_clinical_visit,
            registry=cls.registry,
            content_type=c_type,
            object_id=p1.id,
        )
        p1_ctx2 = RDRFContext.objects.create(
            context_form_group=cls.cfg_symptoms,
            registry=cls.registry,
            content_type=c_type,
            object_id=p1.id,
        )
        p1_ctx3 = RDRFContext.objects.create(
            context_form_group=cls.cfg_symptoms,
            registry=cls.registry,
            content_type=c_type,
            object_id=p1.id,
        )
        p1_ctx4 = RDRFContext.objects.create(
            context_form_group=cls.cfg_sleep,
            registry=cls.registry,
            content_type=c_type,
            object_id=p1.id,
        )
        ...
        for data in (
            ...
            context = RDRFContext.objects.create(
                context_form_group=cls.cfg_tracking,
                registry=cls.registry,
                content_type=ContentType.objects.get_for_model(p3),
                object_id=p3.id,
            )
            ClinicalData.objects.create(
                registry_code="ang",
    ...
    def test_form_section_cde_sort_order(self):
        ...
        cde_keys = [
            "sleep____sleepDiary____timeToBed",
            "sleep____sleepDiary____timesAwoke",
            "recentSymptoms____symptoms____completedDate",
            "recentSymptoms____symptoms____fatigue",
            "recentSymptoms____symptoms____pain",
            "sleep____sleepDiary____dayOfWeek",
        ...
        expected_sort_order = {
            ...
            "recentSymptoms": {
                ...
                "sections": {
                    "symptoms": {
                        "order": 0,
                        "cdes": {"completedDate": 0, "fatigue": 1, "pain": 2},
                    }
                },
            },
        }
    ...
    def test_generate_csv_headers(self):
        ...
        report_design.reportclinicaldatafield_set.create(
            id=1,
            cde_key="sleep____sleepDiary____timeToBed",
            context_form_group=self.cfg_sleep,
        )
        report_design.reportclinicaldatafield_set.create(
            id=2,
            cde_key="sleep____sleepDiary____timesAwoke",
            context_form_group=self.cfg_sleep,
        )
        report_design.reportclinicaldatafield_set.create(
            id=3,
            cde_key="sleep____sleepDiary____difficulty",
            context_form_group=self.cfg_sleep,
        )
        report_design.reportclinicaldatafield_set.create(
            id=4,
            cde_key="recentSymptoms____symptoms____completedDate",
            context_form_group=self.cfg_symptoms,
        )
        report_design.reportclinicaldatafield_set.create(
            id=5,
            cde_key="recentSymptoms____symptoms____fatigue",
            context_form_group=self.cfg_symptoms,
        )
        report_design.reportclinicaldatafield_set.create(
            id=6,
            cde_key="recentSymptoms____otherSymptoms____energy",
            context_form_group=self.cfg_symptoms,
        )
        report_design.reportclinicaldatafield_set.create(
            id=7,
            cde_key="recentSymptoms____symptoms____pain",
            context_form_group=self.cfg_symptoms,
        )
        report_design.reportclinicaldatafield_set.create(
            id=8,
            cde_key="infancy____growth____weight6mo",
            context_form_group=self.cfg_clinical_visit,
        )
    ...
    def test_generate_csv_headers_for_cde_in_multiple_cfgs(self):
        ...
        report_design = ReportDesign.objects.create(
            ...
            cde_heading_format=ReportCdeHeadingFormat.CODE.value,
        )
        report_design.reportclinicaldatafield_set.create(
            id=1,
            cde_key="myHealth____inBed____lieFlat",
            context_form_group=self.cfg_tracking,
        )
        report_design.reportclinicaldatafield_set.create(
            id=2,
            cde_key="myHealth____inBed____needHelp",
            context_form_group=self.cfg_tracking,
        )
        report_design.reportclinicaldatafield_set.create(
            id=3,
            cde_key="myHealth____inBed____mndNeedUse",
            context_form_group=self.cfg_tracking,
        )
