...
class ReportDesignTestCase(TestCase):
    ...
    def test_reports_for_user(self):
        run_reports_perm = Permission.objects.get(
        ...
        registry_1.save()
        report_1 = ReportDesign.objects.create(
            title="Report 1", registry=registry_1
        )
        report_1.access_groups.add(group_clinician)
        report_2 = ReportDesign.objects.create(
            title="Report 2", registry=registry_1
        )
        report_2.access_groups.add(group_clinician)
        report_3 = ReportDesign.objects.create(
            title="Report 3", registry=registry_1
        )
        report_3.access_groups.add(group_curator)

        registry_2 = Registry.objects.create(code="TEST2")
        report_4 = ReportDesign.objects.create(
            title="Report 4", registry=registry_2
        )

        user_no_permissions = CustomUser.objects.create(
            username="standarduser", is_active=True
        )
        user_is_superuser = CustomUser.objects.create(
            username="superuser", is_superuser=True
        )
        user_curator = CustomUser.objects.create(
            username="registry1-curator", is_active=True
        )
        user_curator.registry.add(registry_1)
        user_curator.groups.add(group_curator)
        user_clinician_1 = CustomUser.objects.create(
            username="clinician1", ethically_cleared=False, is_active=True
        )
        user_clinician_1.registry.add(registry_1)
        user_clinician_1.groups.add(group_clinician)
        user_clinician_2 = CustomUser.objects.create(
            username="clinician2", ethically_cleared=True, is_active=True
        )
        ...
        self.assertEqual(
            ...
            self.get_reports_for_user(user_is_superuser),
        )
        ...
        self.assertEqual(
            [report_1, report_2], self.get_reports_for_user(user_clinician_2)
        )
