...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-r", action="store", dest="registry_code", help="registry code"
        )
        parser.add_argument(
            "cmd",
            action="store",
        ...
        if cmd == "list":
            if registry_code is not None:
                try:
                    ...
                    raise CommandError(
                        "Registry with code %s does not exist" % registry_code
                    )

                query = Patient.objects.inactive().filter(
            ...
            if patient_id is None:
                ...
                try:
                    patient_model = Patient.objects.really_all().get(
                        pk=patient_id
                    )
                except Patient.DoesNotExist:
                    raise CommandError(
                        "Patient with id %s does not exist!" % patient_id
                    )
