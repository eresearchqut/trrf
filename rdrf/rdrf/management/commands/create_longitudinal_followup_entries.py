...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--require-consents", type=str, nargs="+", default=[]
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            longitudinal_followup = LongitudinalFollowup.objects.get(
                name=longitudinal_followup_name
            )
        except Registry.DoesNotExist:
            raise CommandError(f"Registry {registry_code} does not exist")
        except LongitudinalFollowup.DoesNotExist:
            raise CommandError(
                f"LongitudinalFollowup {longitudinal_followup_name} does not exist"
            )

        consent_questions = []
        for consent_question in require_consents:
            try:
                consent_questions.append(
                    ConsentQuestion.objects.get(name=consent_question)
                )
            except ConsentQuestion.DoesNotExist:
                raise CommandError(
                    f"ConsentQuestion {consent_question} does not exist"
                )
        ...
        def can_add(patient):
            if (
                ...
                and pending_entries.filter(patient=patient).exists()
            ):
                return False
            ...
            if check_for_form_response:
                ...
                context_response = ClinicalData.objects.filter(
                    django_id=patient, context_id=context
                ).exists()
        ...
        input(
            f"Creating {len(new_entries[:limit])} entries from {len(new_entries)} possible. Press enter to continue"
        )
