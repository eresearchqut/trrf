    ...
    def validate_json(self):
        ...
        if unknown_fields:
            raise ValidationError(
                _("Invalid fields in JSON: {fields}").format(
    ...
    def validate(self):
        ...
        def parse_number(field):
            ...
            try:
                ...
                raise ValidationError(
                    _(
                        "{field} must be {data_type}. Invalid value: {value}"
                    ).format(
                        data_type=cde_datatype,
                        field=field,
        ...
        if cde_min_value is None and min_value is None:
            raise ValidationError(
                _(
                    "You must supply the widget setting Min value if the CDE Min value is not set"
                )
            )
        ...
        if cde_max_value is None and max_value is None:
            raise ValidationError(
                _(
                    "You must supply the widget setting Max value if the CDE Max value is not set"
                )
            )
        ...
        if min_value is not None and cde_min_value is not None:
            if min_value < cde_min_value:
                raise ValidationError(
                    _("Min value must be bigger or equal than CDE's min value!")
                )

        if max_value is not None and cde_max_value is not None:
            if max_value > cde_max_value:
                raise ValidationError(
                    _("Max value must be lower or equal than CDE's max value!")
                )

        if min_value is not None and max_value is not None:
            if max_value <= min_value:
                raise ValidationError(
                    _("Max value should be bigger than Min value")
                )

        if step is not None:
            overall_min_value = (
                cde_min_value if min_value is None else min_value
            )
            overall_max_value = (
                cde_max_value if max_value is None else max_value
            )
            if step >= overall_max_value - overall_min_value:
                raise ValidationError(
                    _("Step value too large for Min value and Max value")
                )
...
class TimeWidgetSettingsValidator(BaseValidator):
    def validate(self):
        ...
        if "format" not in self.settings:
            raise ValidationError(
                _("The format must be specified for time widget settings !")
            )
...
class DurationWidgetSettingsValidator(BaseValidator):
    def validate(self):
        ...
        if all_unset:
            raise ValidationError(
                _("You need to select at least one duration field !")
            )
