...
class OtherPleaseSpecifyWidgetValidator:
    ...
    def validate(self):
            ...
            raise ValidationError(
                {
                    "pv_group": [
                        _(
                            f"{widget_name} widget needs a pv group to be specified !!"
                        )
                    ]
                }
            )
        ...
        if not valid_pv_group:
            valid_groups_qs = CDEPermittedValue.objects.filter(
                value__icontains="specify"
                ...
                raise ValidationError(
                    {
                        "pv_group": [
                            _(
                                "There are no valid pv groups for {widget_name} widget ! A valid pv_group must contain a value with the text 'specify'"
                            )
                        ]
                    }
                )
                ...
                raise ValidationError(
                    {
                        "pv_group": [
                            _(
                                f"The pv group is invalid ! Valid groups are: {valid_groups}"
                            )
                        ]
                    }
                )
