...
from rdrf.services.io.defs.migrations_considered_for_import_export import (
...
def import_export_logic_updates_check(app_configs, **kwargs):
    ...
    for app, name in all_migrations.keys():
        if (
            app in MIGRATIONS_CONSIDERED_FOR_IMPORT_EXPORT
            and name not in MIGRATIONS_CONSIDERED_FOR_IMPORT_EXPORT[app]
        ):
