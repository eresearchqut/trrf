...
def build_patient_filters(filters):
    ...
    operation_input = {
        f"${key}": data_type
        for key, (data_type, variable) in variable_definition.items()
    }
...
def build_patients_query(patient_fields, sort_fields, pagination):
    ...
    patients_input = {
        "sort": f"[{sort_fields_formatted}]",
        "offset": pagination.get("offset"),
        "limit": pagination.get("limit"),
    }
    # remove any inputs with null values
    patients_input = {
        key: val for key, val in patients_input.items() if val is not None
    }
    patient_query = (
        GqlQuery()
        .fields(patient_fields)
        .query("patients", input=patients_input)
        .generate()
    )
...
def build_data_summary_query(data_summary_fields):
    return (
        GqlQuery().fields(data_summary_fields).query("dataSummary").generate()
    )


def build_all_patients_query(
    registry, patient_query_fields, query_input=None, operation_input=None
):
    registry_fields = GqlQuery().fields(patient_query_fields)

    if query_input:
        registry_fields = registry_fields.query(
            "allPatients", input=query_input
        )
    else:
        registry_fields = registry_fields.query("allPatients")

    registry_query = (
        GqlQuery().fields([registry_fields.generate()]).query(registry.code)
    )

    if operation_input:
        registry_query = registry_query.operation(
            name="AllPatientsQuery", input=operation_input
        )
    else:
        registry_query = registry_query.operation()
...
def query_patient_facets(request, registry, facet_keys, filters=None):
    ...
    all_patients_query = build_all_patients_query(
        registry, [facet_query], query_input, operation_input
    )
