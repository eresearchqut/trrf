...
def structure_valid(
    cde_codes, source_section_code, target_section_code, cd_data_dict
):
    print(" Validating Structure ......")
    # Check if 'Forms' exist in cd_data dictionary
    if "forms" not in cd_data_dict.keys():
        print(
            "******* Skipping cdes movement...... No 'forms' found in data dictionary ...... %s"
            % cd_data_dict
        )
        return False
    ...
    if not cd_form:
        print(
            "******* Skipping cdes movement...... Couldn't find 'ClinicalData' form in data dictionary...... %s"
            % cd_data_dict
        )
        return False
    ...
    if not source_section_dict:
        print(
            "******* Skipping cdes movement......Couldn't find source section with code=%s in 'ClinicalData' form: %s"
            % (source_section_code, cd_form)
        )
        return False
    # Check if source section is multi-value
    if not source_section_dict["allow_multiple"]:
        print(
            "******* Skipping cdes movement......Source section is not multi-value : %s "
            % (source_section_dict)
        )
        return False
    ...
    if not target_section_dict:
        print(
            "******* Skipping cdes movement......Couldn't find target section with code=%s in 'ClinicalData' form: %s"
            % (target_section_code, cd_form)
        )
        return False
    # Check if target section is single-value
    if target_section_dict["allow_multiple"]:
        print(
            "******* Skipping cdes movement......Target section is not single-value: %s "
            % (target_section_dict)
        )
        return False
    # Check if cdes (CDE00016,FHCRP) are not in target Section
    cdes_found_in_target_section = [
        cde for cde in target_section_dict["cdes"] if cde["code"] in cde_codes
    ]
    if cdes_found_in_target_section:
        print(
            "******* Skipping cdes movement...... Cdes=%s already exist in Target Section."
            % cdes_found_in_target_section
        )
        return False
    return True
...
def transform_cd_dict(
    cde_codes, source_section_code, target_section_code, cd_data_dict
):
    print(
        "@@@@@@@ Moving Cdes=%s from source section=%s to target section=%s in ClinicalData form @@@@@@@"
...
def move_cdes(cde_codes, source_section, target_section):
    ...
    cdes_to_move = [
        cde for cde in source_section["cdes"][0] if cde["code"] in cde_codes
    ]
    ...
    updated_source_section = [
        clean_cdes(cdes_list, cde_codes) for cdes_list in source_section["cdes"]
    ]
