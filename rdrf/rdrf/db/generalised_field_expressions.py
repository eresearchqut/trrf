    ...
    def __call__(self, patient_model, mongo_data):
        try:
            ...
            logger.error(
                ...
                % (self.__class__.__name__, patient_model.pk, ex)
            )
            return "??ERROR??"
            ...
            if form_dict["name"] == self.form_model.name:
                for section_dict in form_dict["sections"]:
                    if (
                        ...
                        and section_dict["allow_multiple"]
                    ):
                        ...
                        if not section_dict["allow_multiple"]:
                            ...
                            for cde_dict_list in section_dict["cdes"]:
                                ...
                                for cde_dict in cde_dict_list:
                                    cde_map[cde_dict["code"]] = cde_dict[
                                        "value"
                                    ]
...
class AddMultiSectionItemExpression(GeneralisedFieldExpression):
    ...
    def set_value(self, patient_model, mongo_data, item_cde_map, **kwargs):
        ...
        for cde_code in item_cde_map:
            ...
            cde_dict = {"code": cde_code, "value": value}
                ...
                for section_dict in form_dict["sections"]:
                    if section_dict["code"] == self.section_model.code:
                        if section_dict["allow_multiple"]:
                            ...
                            raise Exception(
                                "cannot add an item to a non multisection!"
                            )
        return patient_model, mongo_data
...
class PokeFieldExpression(GeneralisedFieldExpression):
    ...
    def _parse_poke(self, field_expression):
        ...
        _, form_name, section_code, item_num_string, cde_code = (
            field_expression.split("/")
        )
        self.item_number = int(item_num_string)
        self.form_model = RegistryForm.objects.get(
    ...
    def _iterate_through_forms(self, mongo_data, **kwargs):
        ...
        for form_dict in mongo_data["forms"]:
            if form_dict["name"] == self.form_model.name:
                if self.section_model.code not in [
                    x["code"] for x in form_dict["sections"]
                ]:
                    if is_setting:
                        section_dict = {
                            "code": self.section_model.code,
                ...
                for section_dict in form_dict["sections"]:
                    if section_dict["code"] == self.section_model.code:
                        if not section_dict["allow_multiple"]:
                            raise Exception(
                                "Can't get cde in item if not a multisection"
                            )
                        else:
                            ...
                            for item_index, cde_dict_list in enumerate(
                                section_dict["cdes"]
                            ):
                                num = item_index + 1
    ...
    def set_value(self, patient_model, mongo_data, new_value, **kwargs):
        if mongo_data is None:
            if self.item_number == 1:
                item = {"code": self.cde_model.code, "value": new_value}
                ...
                raise ValueError(
                    "Can't update item %s as there is no data"
                    % self.item_number
                )
    ...
    def set_value(self, patient_model, mongo_data, new_value, **kwargs):
        if self.field == "next_of_kin_relationship":
            ...
            if isinstance(new_value, str):
                try:
                    working_group = WorkingGroup.objects.get(
                        registry=self.registry_model, name=new_value
                    )
                    ...
                    raise Exception(
                        "can't update working group on %s" % patient_model
                    )
...
class ConsentExpression(GeneralisedFieldExpression):
    ...
    def evaluate(self, patient_model, mongo_data):
        return patient_model.get_consent(
            self.consent_question_model, self.field
        )
...
class AddressExpression(GeneralisedFieldExpression):
    ...
    def evaluate(self, patient_model, mongo_data):
        try:
            address_model = PatientAddress.objects.get(
                patient=patient_model, address_type=self.address_type_model
            )
...
class AddressesExpression(GeneralisedFieldExpression):
    ...
    def evaluate(self, patient_model, mongo_data):
        return [
            address_object
            for address_object in PatientAddress.objects.filter(
                patient=patient_model
            )
        ]
    ...
    def set_value(self, patient_model, mongo_record, address_maps, **kwargs):
        ...
        for patient_address in PatientAddress.objects.filter(
            patient=patient_model
        ):
...
class ClinicalFormExpression(GeneralisedFieldExpression):
    ...
    def evaluate(self, patient_model, mongo_record):
        return get_cde_value(
            self.form_model, self.section_model, self.cde_model, mongo_record
        )
    ...
    def set_value(self, patient_model, mongo_record, new_value, **kwargs):
        ...
        if context_id is None:
            ...
            default_context_model = (
                context_manager.get_or_create_default_context(patient_model)
            )
        ...
        if not self.section_model.allow_multiple:
            if mongo_record is None:
                ...
                section_dict = {
                    "code": self.section_model.code,
                    "cdes": [{"code": self.cde_model.code, "value": new_value}],
...
class GeneralisedFieldExpressionParser(object):
    ...
    def _parse_ms_expression(self, field_expression):
        try:
            if field_expression.startswith(
                "pick/"
            ) or field_expression.startswith("poke/"):
                return self._parse_poke_expression(field_expression)

            ms_designator, form_name, multisection_code, action_code = (
                field_expression.split("/")
            )
        except ValueError:
        ...
        try:
            ...
            raise FieldExpressionError(
                "Cannot find section %s" % multisection_code
            )

        if action_code == "clear":
            return ClearMultiSectionExpression(
                self.registry_model, form_model, section_model
            )
        elif action_code == "add":
            return AddMultiSectionItemExpression(
                self.registry_model, form_model, section_model
            )
        elif action_code == "items":
            return MultiSectionItemsExpression(
                self.registry_model, form_model, section_model
            )
        else:
            raise FieldExpressionError(
                "ms expression not understood: %s" % field_expression
            )
    ...
    def _parse_consent_expression(self, consent_expression):
        ...
        """
        try:
            _, consent_section_code, consent_code, consent_field = (
                consent_expression.split("/")
            )
        ...
        try:
            consent_section_model = ConsentSection.objects.get(
                code=consent_section_code, registry=self.registry_model
            )
        ...
        try:
            consent_question_model = ConsentQuestion.objects.get(
                code=consent_code, section=consent_section_model
            )
        except ConsentQuestion.DoesNotExist:
            raise FieldExpressionError("consent question does not exist")

        return ConsentExpression(
            self.registry_model, consent_question_model, consent_field
        )
    ...
    def _parse_clinical_form_expression(self, field_expression):
        ...
        return ClinicalFormExpression(
            self.registry_model, form_model, section_model, cde_model
        )
