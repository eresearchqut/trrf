...
class ClinicianSignupRequest(models.Model):
    STATES = (
        ("emailed", "Emailed"),  # clinician emailed
        (
            "signed-up",
            "Signed Up",
        ),  # the clinician accepted the request and a user object was created
        ("created", "Created"),  # request created but nothing sent yet
        ("error", "Error"),  # error
        ("rejected", "Rejected"),
    )  # the clinician received the request but rejected it

    registry = models.ForeignKey(Registry, on_delete=models.CASCADE)
    patient_id = (
        models.IntegerField()
    )  # the patient id whose clinician it is ( had import issues with Patient)
    ...
    clinician_other = models.ForeignKey(
        "patients.ClinicianOther", on_delete=models.CASCADE
    )  # this is the model the parent creates with data about the clinician`1
    ...
    def _get_participants(self, patient):
        ...
        return ",".join(
            ["%s %s" % (pg.first_name, pg.last_name) for pg in participants]
        )
    ...
    def _construct_registration_link(self):
        ...
        return (
            site_url
            + reverse("registration_register", args=(self.registry.code,))
            + "?t=%s" % self.token
        )

    @staticmethod
    def create(registry_model, patient_model, clinician_other, clinician_email):
        csr = ClinicianSignupRequest(
