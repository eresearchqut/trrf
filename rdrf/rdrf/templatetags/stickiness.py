...
def current_registry(request):
    ...
    if (
        ...
        if current_registry != DEFAULT_NAME:
            ...
            try:
                current_registry = Registry.objects.get(
                    id=int(current_registry)
                ).name
...
def sticky_logout_url(request):
    ...
    if registry_name != DEFAULT_NAME:
        try:
            ...
            return reverse(
                "registry", kwargs={"registry_code": registry_object.code}
            )
