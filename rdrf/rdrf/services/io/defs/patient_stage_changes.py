    ...
    def _create_compare_structures(self):
        for s in PatientStage.objects.filter(registry=self.registry):
            self.existing_stages[s.name] = {
                "next_stages": set(ns.name for ns in s.allowed_next_stages.all()),
                "prev_stages": set(ns.name for ns in s.allowed_prev_stages.all())
            }
        ...
        for stage_dict in self.stages:
            ...
            self.import_stages[stage_dict["name"]] = {
                "next_stages": set(self.stages_mapping[sid] for sid in next_stages),
                "prev_stages": set(self.stages_mapping[sid] for sid in prev_stages)
            }

    def _compute_changes(self):
        self._create_compare_structures()
        possible_deleted_stages = self.existing_stages.keys() - self.import_stages.keys()
        possible_new_stages = self.import_stages.keys() - self.existing_stages.keys()
        logger.info(f"possible deleted: {possible_deleted_stages}, possible new: {possible_new_stages}")
