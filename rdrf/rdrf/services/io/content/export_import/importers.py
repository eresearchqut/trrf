...
class DataGroupImporter(object):
    ...
    def import_datagroups(self, datagroups_meta, workdir, **options):
        ...
        for datagroup_meta in datagroups_meta:
            importer = self.catalogue.datagroups.get(
                get_meta_value(datagroup_meta, "name")
            )(self.catalogue)
    ...
    def import_models(self, models_meta, workdir, **options):
        for model_meta in models_meta:
            importer = self.catalogue.models.get(
                get_meta_value(model_meta, "model_name")
            )()
    ...
    def do_import(
        ...
        self.logger.debug(
            "Importing data group '%s'", get_meta_value(datagroup_meta, "name")
        )
        ...
        self.import_datagroups(
            datagroup_meta.get("data_groups", []), workdir, **options
        )
...
class ModelImporter(object):
    ...
    def check_checksum(self, file_name, expected_checksum):
        ...
        if actual_checksum != expected_checksum:
            raise ImportError(
                "Invalid checksum on file '%s'. Actual: '%s', expected: '%s'"
    ...
    def check_object_count(self, model_name, expected, actual):
        if actual != expected:
            raise ImportError(
                "Invalid object_count for model '%s'. Actual: %d, expected: %d"
    ...
    def do_import(
        ...
        **kwargs,
        ...
        file_name = os.path.join(
            workdir, get_meta_value(model_meta, "file_name")
        )
        ...
        with open(file_name) as f:
            ...
            self.check_object_count(
                model_name, object_count, actual_object_count
            )
...
def get_meta_value(meta, key, path=None):
    if "." not in key:
        if key not in meta:
            raise ImportError(
                "Invalid META file. Required entry '%s' is missing."
