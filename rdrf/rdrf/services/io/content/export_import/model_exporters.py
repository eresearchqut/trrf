...
class ModelExporterFilteredByRegistry(ModelExporter):
    ...
    def queryset(self):
        return self.model.objects.filter(
            registry__code=self.exporter_context["registry_code"]
        )
...
class RegistryExporter(ModelExporter):
    ...
    def queryset(self):
        return self.model.objects.filter(
            code=self.exporter_context["registry_code"]
        )
...
class SectionExporter(ModelExporter):
    ...
    def get_sections(registry_code):
        ...
        for section_code in registry.generic_sections:
            section_model = models.Section.objects.filter(
                code=section_code
            ).first()
            if section_model:
                yield section_model
...
class CommonDataElementExporter(ModelExporter):
    @property
    def queryset(self):
        sections = SectionExporter.get_sections(
            self.exporter_context["registry_code"]
        )
        all_unique_cdes_in_sections = set.union(
            *[set(s.cde_models) for s in sections]
        )
...
class EmailNotificationHistoryExporter(ModelExporter):
    ...
    def queryset(self):
        return self.model.objects.filter(
            email_notification__registry__code=self.exporter_context[
                "registry_code"
            ]
        )
...
class WizardExporter(ModelExporter):
    ...
    def queryset(self):
        return self.model.objects.filter(
            registry=self.exporter_context["registry_code"]
        )
...
class ContextFormGroupItemExporter(ModelExporter):
    ...
    def queryset(self):
        return self.model.objects.filter(
            context_form_group__registry__code=self.exporter_context[
                "registry_code"
            ]
        )
...
class CustomUserExporter(ModelExporter):
    ...
    def queryset(self):
        ...
        for user in self.model.objects.filter(registry__code=registry_code):
            user.registry.set(
                models.Registry.objects.filter(code=registry_code)
            )
            user.working_groups.set(
                groupmodels.WorkingGroup.objects.filter(
...
registry_catalogue.register(
    groupmodels.WorkingGroup, ModelExporterFilteredByRegistry
)
registry_catalogue.register(
    models.RegistryForm, ModelExporterFilteredByRegistry
)
...
registry_catalogue.register(
    models.ConsentSection, ModelExporterFilteredByRegistry
)
registry_catalogue.register(
    models.ConsentConfiguration, ModelExporterFilteredByRegistry
)
...
registry_catalogue.register(
    models.DemographicFields, ModelExporterFilteredByRegistry
)
registry_catalogue.register(
    models.EmailNotification, ModelExporterFilteredByRegistry
)
registry_catalogue.register(
    models.EmailNotificationHistory, EmailNotificationHistoryExporter
)
...
registry_catalogue.register(
    models.ContextFormGroup, ModelExporterFilteredByRegistry
)
registry_catalogue.register(
    models.ContextFormGroupItem, ContextFormGroupItemExporter
)
...
registry_catalogue.register(
    patientmodels.ParentGuardian, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.PatientDoctor, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.ClinicianOther, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.PatientStage, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.PatientAddress, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.PatientConsent, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.PatientRelative, ModelExporterFilteredByPatient
)
registry_catalogue.register(
    patientmodels.ConsentValue, ModelExporterFilteredByPatient
)
registry_catalogue.register(models.RDRFContext, ModelExporterFilteredByRegistry)
registry_catalogue.register(
    patientmodels.ClinicianSignupRequest, ModelExporterFilteredByRegistry
)

registry_catalogue.register(models.CDEFile, ModelExporterFilteredByRegistryCode)
registry_catalogue.register(
    models.ClinicalData, ModelExporterFilteredByRegistryCode
)
