...
from .definitions import (
    REGISTRY_DEF_EXPORT_DEFINITION,
    REGISTRY_WITH_DATA_EXPORT_DEFINITION,
)
...
class TopLevelExporter(object):
    ...
    def get_meta_info(self):
        ...
        meta_info.update(
            OrderedDict(
                (("exported_at", self.exported_at), ("data_groups", self.meta))
            )
        )
        return meta_info
    ...
    def zip_it(self):
        ...
        if ext != ".zip":
            raise ValueError(
                "Invalid extension '%s' set on zip_file. Should be '.zip'" % ext
            )
...
class RegistryDefExporter(TopLevelExporter):
    ...
    def export(self, registry_code, **kwargs):
        ...
        logger.info(
            "Registry '%s' exported to '%s'", self.registry_code, self.zip_file
        )
...
class RegistryExporter(RegistryDefExporter):
    ...
    @property
    def zip_file(self):
        return self._zip_file or (
            "exported_%s_with_data.zip" % self.registry_code
        )
