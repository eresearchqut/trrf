...
def handle_longitudinal_followups(user, patient, registry, context_form_group):
    ...
    new_entries = [
        ...
        for longitudinal_followup in LongitudinalFollowup.objects.filter(
            context_form_group=context_form_group
        )
    ]
    logger.debug(
        f"Creating {len(new_entries)} longitudinal followup entries {patient.id=} {context_form_group.id=}"
    )
...
def _get_consents(patient):
    return {
        f"{registry.code}.{section.code}.{question.code}": (
            ...
            if (
                consent_value := ConsentValue.objects.filter(
...
def evaluate_condition(longitudinal_followup_entry):
    if condition := longitudinal_followup_entry.longitudinal_followup.condition:
        try:
            ...
            logger.error(
                f"Error evaluating condition {condition} for {longitudinal_followup_entry.id=}"
            )
...
def _serialize_entries(patient_entries):
    ...
    return {
        longitudinal_followup: list(entries)
        for longitudinal_followup, entries in grouped
    }
...
def send_longitudinal_followups(now):
    allowed_registries = [
        ...
        if r.has_feature(RegistryFeatures.LONGITUDINAL_FOLLOWUPS)
    ]

    outstanding_entries = (
        LongitudinalFollowupEntry.objects.annotate(
            ...
            send_at_debounced=ExpressionWrapper(
                F("send_at") - F("debounce_value"), output_field=DateTimeField()
            ),
            first_form_name=Subquery(
                ContextFormGroupItem.objects.filter(
                    context_form_group=OuterRef(
                        "longitudinal_followup__context_form_group__id"
                    )
                ).values("registry_form__name")[:1]
            ),
            longitudinal_followup_name=F("longitudinal_followup__name"),
            longitudinal_followup_description=F(
                "longitudinal_followup__description"
            ),
            context_form_group_name=F(
                "longitudinal_followup__context_form_group__name"
            ),
            context_form_group_context_type=F(
                "longitudinal_followup__context_form_group__context_type"
            ),
            registry_code=F(
                "longitudinal_followup__context_form_group__registry__code"
            ),
        )
        .filter(
            ...
            patient__id__in=LongitudinalFollowupEntry.objects.filter(
                state=LongitudinalFollowupQueueState.PENDING, send_at__lte=now
            )
            .values("patient__id")
            .distinct(),
        )
    ...
    logger.info(
        f"Found {len(outstanding_entries)} outstanding followup entries"
    )

    sent_success = sent_failure = 0
    for patient_id, patient_entries_group in itertools.groupby(
        outstanding_entries, lambda entry: entry.patient.id
    ):
        all_patient_entries = list(patient_entries_group)
        patient_entries = list(filter(evaluate_condition, all_patient_entries))
        logger.debug(
            f"Patient {patient_id}: {len(patient_entries)} / {len(all_patient_entries)} followup entries"
        )
        ...
        if not patient_registry.has_feature(
            RegistryFeatures.LONGITUDINAL_FOLLOWUPS
        ):
            logger.info(
                f"Halting longitudinal followup processing as registry {registry_code} disabled the feature"
            )
            break
        ...
        LongitudinalFollowupEntry.objects.bulk_update(
            patient_entries, ["sent_at", "state"]
        )
    ...
    logger.info(
        f"Sent {sent_success} followup emails, failed to send {sent_failure}"
    )
