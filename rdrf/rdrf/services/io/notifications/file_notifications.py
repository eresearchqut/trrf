...
def handle_file_notifications(registry, patient, storage):
    ...
    notifications = EmailNotification.objects.filter(registry=registry, description=EventType.FILE_UPLOADED)
    if not notifications.exists():
        return

    for notification in notifications:
        uploads = [u for u in uploaded_cde_files if _notification_interested_in_upload(notification, u)]
        if uploads:
            template_data = _setup_template_data(registry, patient, uploads)
            process_given_notification(notification, template_data=template_data)
...
def _cde_info(registry, form_name, section_code, cde_code):
    ...
    display_name = ' --- '.join((form_display_name, section_display_name, cde_display_name))
