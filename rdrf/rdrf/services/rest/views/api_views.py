        ...
        if updated_since := self.request.query_params.get("updated_since"):
            if updated_since.isdigit():
                ...
                return patients.filter(
                    last_updated_overall_at__gt=updated_since
                )
            else:
...
class ListCountries(APIView):
    ...
    def get(self, request, format=None):
        ...
        def to_dict(country):
            ...
            d["states"] = reverse(
                "state_lookup", args=[country.alpha_2], request=request
            )
...
class LookupIndex(APIView):
    ...
    def get(self, request, registry_code, format=None):
        ...
        query = (
            Q(given_names__icontains=term) | Q(family_name__icontains=term)
        ...
        return Response(
            list(
                map(
                    to_dict,
                    [p for p in Patient.objects.filter(query) if p.is_index],
                )
            )
        )
...
class PatientStages(generics.ListAPIView):
    ...
    def registry(self):
        ...
        if registry is None or not registry.has_feature(
