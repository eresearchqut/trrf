...
EMAIL_CHANGE_REQUEST_EXPIRY_SECONDS = (
    60 * 60 * EMAIL_CHANGE_REQUEST_EXPIRY_HOURS
)
...
def initiate_email_change_request(
    user, new_email_address, requires_activation=True
):
...
def activate_email_change_request(user):
    if (
        _user_has_email_change_request(user)
        and user.emailchangerequest.status == EmailChangeRequestStatus.PENDING
    ):
        _sync_user_email_update(user, user.emailchangerequest.new_email)
        _complete_change_request(user)
        _send_email_change_request_completed_notification(
            user, user.emailchangerequest.current_user_email
        )
...
def _create_email_change_request(user, new_email_address):
    ...
    user.emailchangerequest = EmailChangeRequest(
...
def _send_email_change_request_notification(user):
    ...
    token_authenticated_link = make_token_authenticated_link(
        viewname="activate_email_request", username=user.username, token=token
    )
    activation_link = make_full_url(token_authenticated_link)
    email_recipient = {
        user.emailchangerequest.new_email: user.preferred_language
    }

    process_notification(
        reg_code=user.registry_code,
...
def _send_email_change_request_completed_notification(
    user, user_previous_email
):
...
def _sync_user_email_update(user, new_email_address):
    ...
    user_email_updated.send(
        sender=_sync_user_email_update, user=user, previous_email=previous_email
    )
