...
from rdrf.testing.behaviour.features.auth.page import (
    BasePage,
...
def go_to_url(_step, page_name):
    page_url = get_site_links(world.site_url, world.registry_code).get(
        page_name
    )
...
@step(
    'try to register as a user called "([^"]+)" using the email address "([^"]+)" and the password "([^"]+)"'
)
def patient_self_registration(_step, client_name, email_address, password):
    ...
    captcha_iframe_element = world.browser.find_element(
        by=By.XPATH, value="//iframe[@role='presentation']"
    )
...
def login(_step, username, password):
    LoginPage(world.browser).set_username(username).set_password(
        password
    ).submit()
...
def set_new_password(_step, new_password):
    ResetPasswordPage(world.browser).set_new_password(
        new_password
    ).set_confirm_password(new_password).submit()
...
def assert_is_time_based_key_displayed(_step):
    time_based_key = TwoFactorTokenGeneratorPage(
        world.browser
    ).get_time_based_key()
