...
def assert_current_username(_step, expected_current_email):
    actual_current_email = ChangeEmailAddressPage(world.browser).get_current_email()
...
def set_confirm_email_address(_step, confirm_email_address):
    ChangeEmailAddressPage(world.browser).set_confirm_new_email(confirm_email_address)
...
@step('my "Current Email Change Request" is displayed, with the following details:')
def assert_current_email_request(_step):
    current_request = ChangeEmailAddressPage(world.browser).get_current_request()
    for i, expected_message in enumerate(guess_types(_step.hashes)):
        assert_equal(current_request['email'], expected_message['Requested email address'])
