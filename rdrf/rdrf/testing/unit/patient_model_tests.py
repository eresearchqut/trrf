...
class PatientAgeTest(RDRFTestCase):
    ...
    def _create_patient(self, registry, date_of_birth):
        p = Patient.objects.create(consent=True, date_of_birth=date_of_birth)
    ...
    def test_living_patient_age(self, *args, **kwargs):
        ...
        self.assertEqual(
            self._get_calculated_age(self.patient, date(2019, 1, 1)), 3
        )
        self.assertEqual(
            self._get_calculated_age(self.patient, date(2019, 12, 2)), 2
        )  # birthday month/day greater than today
        self.assertEqual(
            self._get_calculated_age(self.patient, date(2020, 2, 29)), 2
        )  # Valid leap year, "current" year (2022) is not a leap year.
    ...
    def test_deceased_patient_age(self, *args, **kwargs):
        ...
        self.assertEqual(
            self._get_calculated_age(
                ...
                date_of_death=date(1999, 7, 24),
            ),
            28,
        )  # birthday month/day greater than DOD
        self.assertEqual(
            self._get_calculated_age(
                self.patient,
                date_of_birth=date(2000, 2, 29),
                date_of_death=date(2005, 5, 24),
            ),
            5,
        )  # Valid leap year, DOD year is not a leap year.
        self.assertEqual(
            self._get_calculated_age(
                self.patient,
                date_of_birth=date(2000, 2, 29),
                date_of_death=date(2008, 5, 24),
            ),
            8,
        )  # Valid leap year, DOD year is a leap year.
