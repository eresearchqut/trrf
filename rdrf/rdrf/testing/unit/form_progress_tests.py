...
from rdrf.models.definition.models import ClinicalData, CommonDataElement
...
class FormProgressTestCase(FormTestCase):
    def create_sections(self):
        ...
        self.sectionE = self.create_section(
            "sectionE",
            "Section E",
            ["DM1ChronicInfection", "DM1Cholesterol", "DateOfAssessment"],
            False,
        )
    ...
    def create_forms(self):
        ...
        self.new_form = self.create_form(
            "NewForm",
            [self.sectionA, self.sectionB, self.sectionD, self.sectionE],
        )
    ...
    def _compute_progress(self, form, progress_cdes_map):
        fpc = FormProgressCalculator(
            form.registry, form, self._get_clinical_data(), progress_cdes_map
        )
    ...
    def test_form_progress_simple_condition(self):
        ...
        progress_cdes_map[self.new_form.name] = set(
            cde_model.code
            for cde_model in self.new_form.complete_form_cdes.all()
        )
    ...
    def test_form_progress_section_condition(self):
        ...
        cde_name = CommonDataElement.objects.get(code="CDEName")
        cde_fatigue = CommonDataElement.objects.get(code="DM1Fatigue")
        cde_fatigue_reading = CommonDataElement.objects.get(
            code="DM1FatigueSittingReading"
        )
        ...
        progress_cdes_map[self.other_form.name] = set(
            cde_model.code
            for cde_model in self.other_form.complete_form_cdes.all()
        )
