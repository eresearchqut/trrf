...
class CustomUserTest(TestCase):
    def test_get_full_name(self):
        ...
        user = CustomUser.objects.create(
            first_name="Valerie", last_name="Brother"
        )
        ...
        ParentGuardian.objects.create(
            first_name="Valorie", last_name="Sister", user=user
        )
    ...
    def test_default_page(self):
        def _execute_test_cases(test_cases):
            for test_case in test_cases:
                ...
                self.assertEqual(
                    user.default_page,
                    expected_page,
                    f"{group=} {user_registry=}",
                )

        registry = Registry.objects.create(code="test")
        user = CustomUser.objects.create()
        Patient.objects.create(
            consent=True, date_of_birth="1980-01-02", user=user
        )
        ...
        test_cases = [
            (None, None, page_patientslisting),
        ...
        patient = Patient.objects.create(
            consent=True, date_of_birth="1980-01-02"
        )
...
class GroupTest(TestCase):
    ...
    def test_patient(self):
        self._test_group_attrs(
            [GROUPS.PATIENT], ["is_patient", "is_patient_or_delegate"]
        )

    def test_parent(self):
        self._test_group_attrs(
            [GROUPS.PARENT], ["is_parent", "is_patient_or_delegate"]
        )

    def test_carrier(self):
        self._test_group_attrs(
            [GROUPS.CARRIER], ["is_carrier", "is_patient_or_delegate"]
        )

    def test_carer(self):
        self._test_group_attrs(
            [GROUPS.CARER], ["is_carer", "is_patient_or_delegate"]
        )
    ...
    def test_working_group_staff(self):
        self._test_group_attrs(
            [GROUPS.WORKING_GROUP_STAFF], ["is_working_group_staff"]
        )
    ...
    def test_patient_with_custom_group(self):
        self._test_group_attrs(
            [GROUPS.PATIENT, "custom_group"],
            ["is_patient", "is_patient_or_delegate"],
        )
