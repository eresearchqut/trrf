...
class WidgetTest(TestCase):
    def test_xnat_extract_lookup_values(self):
        self.assertEqual(
            XnatWidget.extract_lookup_values("PROJ1;SUBJ-Y"),
            ["PROJ1", "SUBJ-Y"],
        )
        self.assertEqual(
            XnatWidget.extract_lookup_values("Assessment ABC;Patient XYZ"),
            ["Assessment ABC", "Patient XYZ"],
        )
        self.assertEqual(
            XnatWidget.extract_lookup_values(";Subject"), ["", "Subject"]
        )
