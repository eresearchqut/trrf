...
class CodeGenTestCase(FormTestCase):
    ...
    def visibility_handler_basic_validation(self, visibility_handler_output):
        ...
        assert (
            "visibility_map_update(visibility_map, "
            in visibility_handler_output
        )

    def visibility_handler_simple_test(
        self, visibility_handler_output, name, op, value
    ):
        assert (
            f'test_cde_value_simple("{name}", "{op}", "{value}")'
            in visibility_handler_output
        )
    ...
    def test_code_gen_with_simple_dsl(self):
        ...
        self.change_handler_validation(
            'get_cde_name("DM1Cholesterol____CDEAge", 0)', generated_js
        )
        vho = code_gen.generate_visibility_handler()
        self.visibility_handler_basic_validation(vho)
        self.visibility_handler_simple_test(
            vho, "DM1Cholesterol____CDEAge", "==", "10"
        )
    ...
    def test_three_or_conditions(self):
        ...
        self.basic_generated_code_validation(generated_js)
        self.change_handler_validation(
            'get_cde_name("sectionD____DM1FatigueSittingReading", 0)',
            generated_js,
        )
        self.change_handler_validation(
            'get_cde_name("sectionD____CDEfhDrugs", 0)', generated_js
        )
        self.change_handler_validation(
            'get_cde_name("sectionD____CDEfhHistoryDrugIntolerance", 0)',
            generated_js,
        )
        ...
        self.visibility_handler_simple_test(
            vho,
            "sectionD____DM1FatigueSittingReading",
            "==",
            "DM1FatigueDozingSlightChance",
        )
        self.visibility_handler_simple_test(
            vho, "sectionD____CDEfhDrugs", "==", "PVfhDrugsEzetimibe"
        )
        self.visibility_handler_simple_test(
            vho, "sectionD____CDEfhHistoryDrugIntolerance", "==", "y"
        )
    ...
    def test_multi_section_condition_and_targets_same_section_with_section_prefix(
        ...
        assert (
            "test_cde_value(name, 'sectionF____DM1Apathy', '==', 'Yes')" in vho
        )
        change_targets_output = code_gen.generate_change_targets()
        assert (
            'change_handler.hasOwnProperty("formset_sectionF")'
            in change_targets_output
        )
        assert (
            'change_handler["formset_sectionF"].push("sectionF____DM1Apathy")'
            in change_targets_output
        )
        assert (
            'change_handler["formset_sectionF"] = ["sectionF____DM1Apathy"]'
            in change_targets_output
        )
    ...
    def test_code_gen_with_simple_dsl_multiple_change_handlers(self):
        ...
        assert (
            generated_js.count('get_cde_name("DM1Cholesterol____CDEAge", 0)')
            == 1
        )
