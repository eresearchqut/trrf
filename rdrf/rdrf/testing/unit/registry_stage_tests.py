...
from rdrf.services.io.defs.importer import (
    Importer,
    ImportState,
    RegistryImportError,
)
...
class RegistryStageTests(RDRFTestCase):
    ...
    def test_stage_and_rule_creation_on_feature_add(self):
        ...
        self.registry.save()
        self.assertTrue(
            PatientStage.objects.filter(registry=self.registry).exists()
        )
        self.assertEqual(
            PatientStage.objects.filter(registry=self.registry).count(), 7
        )
        self.assertIsNotNone(
            PatientStage.objects.filter(
                registry=self.registry, name="Trial"
            ).first()
        )
        self.assertTrue(
            PatientStageRule.objects.filter(registry=self.registry).exists()
        )
        self.assertEqual(
            PatientStageRule.objects.filter(registry=self.registry).count(), 2
        )
    ...
    def _export(self):
        ...
        self.assertEqual(
            len(errors), 0, "Expected zero errors instead got:%s" % errors
        )
        self.assertTrue(
    ...
    def test_stage_import_export_delete_stage(self):
        ...
        PatientStage.objects.create(
            name="New test stage", registry=self.registry
        )

        existing_stage_ids = set(
            PatientStage.objects.filter(registry=self.registry).values_list(
                "pk", flat=True
            )
        )
        existing_rule_ids = set(
            PatientStageRule.objects.filter(registry=self.registry).values_list(
                "pk", flat=True
            )
        )
        importer = Importer()
    ...
    def test_stage_import_export_rename_stage(self):
        ...
        PatientStage.objects.filter(
            name="Trial", registry=self.registry
        ).update(name="Trial2")
        ...
        self.assertFalse(
            PatientStage.objects.filter(
                name="Trial2", registry=self.registry
            ).exists()
        )
        self.assertTrue(
            PatientStage.objects.filter(
