...
from rdrf.auth.password_validation import (
    ConsecutivelyDecreasingNumberValidator,
    ConsecutivelyIncreasingNumberValidator,
    ConsecutivelyRepeatingCharacterValidator,
    EnhancedCommonPasswordValidator,
...
class PasswordValidationTests(TestCase):
    ...
    @mock.patch(
        f"{password_validation.__name__}.pwned_passwords", wraps=pwned_passwords
    )
    def test_insecure_validation(self, *args, **kwargs):
        ...
        validator = EnhancedCommonPasswordValidator(
        ...
        validator = EnhancedCommonPasswordValidator(
            breached_password_detection=True, max_breach_threshold=1
        )
        password_validation.pwned_passwords.check_breaches = Mock(
            return_value=52632
        )
        with self.assertRaises(ValidationError):
            validator.validate("Password12!")
        password_validation.pwned_passwords.check_breaches.assert_called_once_with(
            "Password12!"
        )
    ...
    def test_pwnedpasswords_api_is_down(self, *args, **kwargs):
        ...
        password_validation.pwned_passwords.check_breaches = Mock(
            side_effect=Exception("Mock API Exception")
        )
