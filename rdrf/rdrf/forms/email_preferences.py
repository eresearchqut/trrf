...
from rdrf.models.definition.models import EmailNotification, EmailPreference
...
def unsubscribe_all_choices():
    return [
        (True, _("I would like to unsubscribe from all emails")),
...
class EmailPreferencesForm(ModelForm):
    ...
    class Meta:
        ...
        exclude = ("user",)
        widgets = {
            "unsubscribe_all": RadioSelect(
                choices=unsubscribe_all_choices(),
    ...
    def setup_initials(self):
        if self.instance.id:
            self.fields[
                "unsubscribe_all"
            ].initial = self.instance.unsubscribe_all
            ...
            self.fields["email_notifications"].initial = [
                ...
                if self.instance.unsubscribe_all
                or self.instance.is_email_allowed(email_notification)
            ]
        else:
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.user = user
        self.instance = (
            EmailPreference.objects.get_by_user(user) or self.instance
        )
    ...
    def clean_email_notifications(self):
        return [
            EmailNotification.objects.get(id=id)
            for id in self.cleaned_data["email_notifications"]
        ]
