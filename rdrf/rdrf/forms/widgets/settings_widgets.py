...
class JSONWidgetSettings(Widget):
    ...
    def parse_value(self, value):
        ...
        try:
            ...
            self.parsed = {k: v for k, v in self.parsed.items() if k in self.get_allowed_fields()}
        except Exception:
            pass
...
class SliderWidgetSettings(JSONWidgetSettings):
    ...
    def generate_inputs(self):
        self.generate_text_input('min', _('Min value'), info=_("leave empty if you want to use the CDE's min value")),
        self.generate_text_input('max', _('Max value'), info=_("leave empty if you want to use the CDE's max value"))
...
class TimeWidgetSettings(JSONWidgetSettings):
    ...
    def generate_input(self, name, title, info=None):
        ...
        self.generate_select_input(
            ...
            options=[
                {"value": TimeWidget.AMPM, "selected": selected_12hour, "text": "12-hour-format"},
                {"value": TimeWidget.FULL, "selected": selected_24hour, "text": "24-hour-format"}
            ]
        )
...
class RadioSelectSettings(JSONWidgetSettings):
    ...
    def generate_inputs(self):
        self.generate_input(
            'force_vertical', _('Force vertical layout'),
