...
NZ_STATES = _l(
    "NZ-TKI/NZ-CIT/NZ-NSN/NZ-BOP/NZ-WTC/NZ-GIS/NZ-CAN/NZ-MBH/NZ-HKB/NZ-WGN/NZ-STL/NZ-MWT/NZ-N/NZ-WKO/NZ-OTA/NZ-S/NZ-TAS/NZ-AUK/NZ-NTL"
)
...
class SpreadSheetCreator(object):
    ...
    def create(self):
        self.workbook = xl.Workbook(
            self.output_filename, {"default_date_format": "dd/mm/yyyy"}
        )
    ...
    def _patient_demographics_spec(self):
        return [
            ...
            (
                "Consent given to store data only while individual is living",
                BOOL,
            ),
            (
                "Consent given to store data for the duration of the registry",
                BOOL,
            ),
            (
                "Consent provided by Parent/Guardian only while individual is living",
                BOOL,
            ),
            (
                "Consent provided by Parent/Guardian for the duration of the registry",
                BOOL,
            ),
    ...
    def _get_type(self, cde_dict):
        ...
        if datatype in ["string", "text"]:
            ...
            raise Exception(
                "Unknown datatype for %s: %s" % (cde_dict, datatype)
            )
